  ...
  if (t.isObjectExpression(css)) {
    ...
    css.properties = css.properties.reduce(function propertiesReducer(acc, property) {
      if (t.isObjectExpression(property.value)) {
        ...
        acc.push(t.objectProperty(property.key, t.memberExpression(p, internalName)));
      } else {
    ...
    css.expressions = css.expressions.reduce((acc, ex) => {
      if (
        Object.keys(bindings).some(key => bindings[key].referencePaths.find(p => p.node === ex)) ||
        ...
        elem.node.attributes.push(
          t.jSXAttribute(t.jSXIdentifier(internalName.name), t.jSXExpressionContainer(ex)),
        );

        acc.push(t.arrowFunctionExpression([p], t.memberExpression(p, internalName)));
      }
